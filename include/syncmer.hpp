 ...
 * \tparam t The offset for the position of the smallest sub-window. Default: 0.
 ...
 */
template <std::ranges::view urng1_t, std::ranges::view urng2_t, bool opensyncmer = false, size_t t = 0>
...
template <std::ranges::view urng1_t, std::ranges::view urng2_t, bool opensyncmer, size_t t>
template <bool const_range>
class syncmer_view<urng1_t, urng2_t, opensyncmer, t>::basic_iterator
{
private:
...
private:
    ...
    void window_first(const size_t window_size)
    {
        ...
        {
            if (syncmer_position_offset == t || syncmer_position_offset == w_size - 1 )
            {
                auto syncmer_it = urng2_iterator;
                syncmer_value = *syncmer_it;
            }
    ...
    bool next_syncmer()
    {
        ...
        else if (syncmer_position_offset == (t+1))
        {
            auto syncmer_it = urng2_iterator;
            syncmer_value = *syncmer_it;
...
syncmer_view(rng1_t &&, rng2_t &&, size_t const window_size) -> syncmer_view<std::views::all_t<rng1_t>, std::views::all_t<rng2_t>>;

template <std::ranges::viewable_range rng1_t, std::ranges::viewable_range rng2_t, bool opn>
syncmer_view(rng1_t &&, rng2_t &&, size_t const window_size) -> syncmer_view<std::views::all_t<rng1_t>, std::views::all_t<rng2_t>, opn>;

template <std::ranges::viewable_range rng1_t, std::ranges::viewable_range rng2_t, bool opn, size_t t>
syncmer_view(rng1_t &&, rng2_t &&, size_t const window_size) -> syncmer_view<std::views::all_t<rng1_t>, std::views::all_t<rng2_t>, opn, t>;

template <std::ranges::viewable_range rng1_t, std::ranges::viewable_range rng2_t, size_t t>
syncmer_view(rng1_t &&, rng2_t &&, size_t const window_size) -> syncmer_view<std::views::all_t<rng1_t>, std::views::all_t<rng2_t>, t>;
...
struct syncmer_fn
{
    ...
    constexpr auto operator()(urng2_t urange2, const size_t window_size) const
    {
