 ...
 *
 * \tparam opensyncmer If false, syncmers are used but if ture, open-syncmers are used. Default: False.
 ...
 */
template <std::ranges::view urng1_t, std::ranges::view urng2_t, bool opensyncmer = false>
...
public:
    ...
    * \param[in] t           The offset for the position of the smallest sub-window.
    */
    syncmer_view(urng1_t urange1, urng2_t urange2, size_t const window_size, size_t const t) :
    ...
    */
    ...
    //!\endcond
    syncmer_view(other_urng1_t && urange1, other_urng2_t && urange2, size_t const window_size, size_t const t) :
...
template <std::ranges::view urng1_t, std::ranges::view urng2_t, bool opensyncmer>
template <bool const_range>
class syncmer_view<urng1_t, urng2_t, opensyncmer>::basic_iterator
{
private:
...
syncmer_view(rng1_t &&, rng2_t &&, size_t const window_size, size_t const t) -> syncmer_view<std::views::all_t<rng1_t>, std::views::all_t<rng2_t>>;

template <std::ranges::viewable_range rng1_t, std::ranges::viewable_range rng2_t, bool opn>
syncmer_view(rng1_t &&, rng2_t &&, size_t const window_size, size_t const t) -> syncmer_view<std::views::all_t<rng1_t>, std::views::all_t<rng2_t>, opn>;
// ---------------------------------------------------------------------------------------------------------------------
...
struct syncmer_fn
{
    ...
    constexpr auto operator()(urng2_t urange2, const size_t window_size, size_t const t) const
    {
        return adaptor_from_functor{*this, urange2, window_size, t};
    }
    ...
    constexpr auto operator()(urng1_t && urange1, urng2_t && urange2, size_t const window_size, size_t const t) const
    {
        ...
        if (t >= window_size)
            throw std::invalid_argument{"The chosen t is too short to satisfy the given window_size.\n"
                                        "Please choose a smaller t."};

        return syncmer_view{urange1, urange2, window_size, t};
    }
};
//![adaptor_def]
 ...
 * its smallest subwindow of size s, (s < k) at its start or end. An open-syncmer has its smallest subwindow
 * at its start.
 ...
 * `ACGGCGACGTT` and 5 as `window size`, 2 as `subwindow size`, the closed-syncmer values are
 * `ACGGC,CGGCG,GCGAC,ACGTT` and the open-syncmer values are `ACGGC,CGGCG,ACGTT`.
 *
 *
 *
 * ### View properties
