 ...
 * \brief Provides seqan3::views::minimiser_distance.
 */
...
{
// ---------------------------------------------------------------------------------------------------------------------
// minimiser_distance_view class
// ---------------------------------------------------------------------------------------------------------------------

/*!\brief The type returned by seqan3::views::minimiser_distance.
 ...
 * \details
 *
 * See seqan3::views::minimiser_distance for a detailed explanation on minimizers.
 *
 * \note Most members of this class are generated by std::ranges::view_interface which is not yet documented here.
 *
 * \sa seqan3::views::minimiser_distance
 */
template <std::ranges::view urng1_t,
          std::ranges::view urng2_t = std::ranges::empty_view<seqan3::detail::empty_type>>
class minimiser_distance_view : public std::ranges::view_interface<minimiser_distance_view<urng1_t, urng2_t>>
{
private:
    static_assert(std::ranges::forward_range<urng1_t>, "The minimiser_distance_view only works on forward_ranges.");
    static_assert(std::ranges::forward_range<urng2_t>, "The minimiser_distance_view only works on forward_ranges.");
    ...
    //!\brief The sentinel type of the minimiser_distance_view.
...
public:
    ...
    minimiser_distance_view & operator=(minimiser_distance_view const & rhs) = default; //!< Defaulted.
    minimiser_distance_view & operator=(minimiser_distance_view && rhs) = default; //!< Defaulted.
    ~minimiser_distance_view() = default; //!< Defaulted.
    ...
    */
    minimiser_distance_view(urng1_t urange1, size_t const window_size) :
        minimiser_distance_view{std::move(urange1), default_urng2_t{}, window_size}
    {}
    ...
    */
    ...
    //!\endcond
    minimiser_distance_view(other_urng1_t && urange1, size_t const window_size) :
    ...
    * \param[in] window_size The number of values in one window.
    */
    minimiser_distance_view(urng1_t urange1, urng2_t urange2, size_t const window_size) :
    ...
    */
    ...
    //!\endcond
    minimiser_distance_view(other_urng1_t && urange1, other_urng2_t && urange2, size_t const window_size) :
        urange1{std::views::all(std::forward<other_urng1_t>(urange1))},
...
//!\brief Iterator for calculating minimiser_distances.
template <std::ranges::view urng1_t, std::ranges::view urng2_t>
template <bool const_range>
class minimiser_distance_view<urng1_t, urng2_t>::basic_iterator
{
private:
...
public:
    ...
    //!\endcond
        : minimiser_distance_value{std::move(it.minimiser_distance_value)},
    ...
    * \details
    *
    * Looks at the number of values per window in two ranges, returns the smallest between both as minimiser_distance and
    * shifts then by one to repeat this action. If a minimiser_distance in consecutive windows is the same, it is returned only
    * once.
    */
    ...
    //!\brief Compare to the sentinel of the minimiser_distance_view.
    ...
    {
        next_unique_minimiser_distance();
        return *this;
    }
    ...
    //!\brief Return the minimiser_distance.
    value_type operator*() const noexcept
    {
        return minimiser_distance_value;
    }
...
private:
    ...
    //!\brief Stored values per window. It is necessary to store them, because a shift can remove the current minimiser_distance.
    std::deque<value_type> window_values{};

    //!\brief Increments iterator by 1.
    void next_unique_minimiser_distance()
    {
        while (!next_minimiser_distance()) {}
    }
    ...
    //!\brief Calculates minimiser_distances for the first window.
     ...
     * \returns True, if new minimiser_distance is found or end is reached. Otherwise returns false.
     * \details
...
minimiser_distance_view(rng1_t &&, size_t const window_size) -> minimiser_distance_view<std::views::all_t<rng1_t>>;

//!\brief A deduction guide for the view class template.
template <std::ranges::viewable_range rng1_t, std::ranges::viewable_range rng2_t>
minimiser_distance_view(rng1_t &&, rng2_t &&, size_t const window_size) -> minimiser_distance_view<std::views::all_t<rng1_t>,
                                                                                        std::views::all_t<rng2_t>>;

// ---------------------------------------------------------------------------------------------------------------------
// minimiser_distance_fn (adaptor definition)
// ---------------------------------------------------------------------------------------------------------------------

//![adaptor_def]
//!\brief seqan3::views::minimiser_distance's range adaptor object type (non-closure).
//!\ingroup search_views
struct minimiser_distance_fn
{
    ...
    constexpr auto operator()(urng1_t && urange1, size_t const window_size) const
    {
        static_assert(std::ranges::viewable_range<urng1_t>,
                      "The range parameter to views::minimiser_distance cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng1_t>,
                      "The range parameter to views::minimiser_distance must model std::ranges::forward_range.");
        ...
        return minimiser_distance_view{urange1, window_size};
    }
};
//![adaptor_def]
