 ...
 * \brief Provides randstrobe_hash.
 */
...
{
//!\brief seqan3::views::randstrobe2_hash's range adaptor object type (non-closure).
//!\ingroup search_views
struct randstrobe2_hash_fn
{
    ...
    */
                              ...
                              seed const seed = seqan3::seed{0x8F3F73B5CF1C9ADE}) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
            "The range parameter to views::randstrobe_hash cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
            "The range parameter to views::randstrobe_hash must model std::ranges::forward_range.");
        static_assert(semialphabet<std::ranges::range_reference_t<urng_t>>,
            "The range parameter to views::randstrobe_hash must be over elements of seqan3::semialphabet.");
        ...
        auto randstrobes = seqan3::detail::randstrobe_view(hashed_values, window_min + shape.size() - 1, window_len - shape.size() + 1);
        uint64_t multiplicator = std::pow(4, shape.count());
        auto forward = std::views::transform(randstrobes, [multiplicator] (std::vector<uint64_t> i)
        ...
        auto rev_randstrobes = std::views::reverse(seqan3::detail::randstrobe_view(rev_hashed_values, window_min + shape.size() - 1, window_len - shape.size() + 1));
        auto reverse = std::views::transform(rev_randstrobes, [multiplicator] (std::vector<uint64_t> i)
                          {return combine_strobes(multiplicator, i[0], i[1]);});
...
//!\brief seqan3::views::randstrobe3_hash's range adaptor object type (non-closure).
//!\ingroup search_views
struct randstrobe3_hash_fn
{
    /*!\brief Store the shape and the window min and max offsets and return a range adaptor closure object.
    * \param[in] shape       The seqan3::shape to use for hashing.
    * \param[in] window_min  The lower offset for the position of the next window from the previous one.
    ...
    */
                              ...
                              seed const seed = seqan3::seed{0x8F3F73B5CF1C9ADE}) const
    {
        ...
        auto randstrobes = seqan3::detail::randstrobe_view<decltype(hashed_values), 3>(hashed_values, window_min + shape.size() - 1, window_len - shape.size() + 1);
        uint64_t multiplicator = std::pow(4, shape.count()*2);
        uint64_t multiplicator2 = std::pow(4, shape.count());
        auto forward = std::views::transform(randstrobes, [multiplicator, multiplicator2] (std::vector<uint64_t> i)
...
/*!\brief                    Computes randstrobes for a range with a given shape, min and max window offsets and seed.
 ...
 * \returns                  A range of `size_t` where each value is a vector composed of two randstrobes.
 ...
 *
 */
inline constexpr auto randstrobe2_hash = seqan3::detail::randstrobe2_hash_fn{};
 ...
 *
 */
inline constexpr auto randstrobe3_hash = seqan3::detail::randstrobe3_hash_fn{};
