...
using iterator_type = std::ranges::iterator_t<decltype(std::declval<seqan3::dna4_vector&>()
                                                       | modmer_hash_distance(seqan3::ungapped{4},
                                                                     2,
                                                                     seqan3::seed{0}))>;

static constexpr seqan3::shape ungapped_shape = seqan3::ungapped{4};
static constexpr seqan3::shape gapped_shape = 0b1001_shape;
static constexpr auto ungapped_view = modmer_hash_distance(ungapped_shape,
                                                  2,
                                                  seqan3::seed{0});
static constexpr auto gapped_view = modmer_hash_distance(gapped_shape,
                                                2,
                                                seqan3::seed{0});
...
struct iterator_fixture<iterator_type> : public ::testing::Test
{
    ...
    result_t expected_range{6, 1, 0, 0};
...
template <typename T>
class modmer_hash_distance_view_properties_test: public ::testing::Test { };
...
TYPED_TEST_SUITE(modmer_hash_distance_view_properties_test, underlying_range_types, );

class modmer_hash_distance_test : public ::testing::Test
{
protected:
    ...
    result_t result3{6, 1, 0, 0};  // ACGT/ACGT, GTTT/AAAC, TTTA/TAAA, TTAG/CTAA // A--T/A--T, G--T/A--C, T--A/T--A, T--G/C--A - "-" for gap
    result_t result3_stop{};
    result_t result3_start{0, 1, 0, 0};
};
...
TYPED_TEST(modmer_hash_distance_view_properties_test, different_input_ranges)
{
    ...
    result_t ungapped{0, 2, 2, 1, 0, 0}; // ACGT/ACGT, TCGA/TCGA, ACGT/ACGT, GTTT/AAAC, TTTA/TAAA, TTAG/CTAA
    result_t gapped{0, 2, 2, 1, 0, 0};   // A--T/A--T, T--A/T--A, A--T/A--T, G--T/A--C, T--A/T--A, T--G/C--A - "-" for gap
    EXPECT_RANGE_EQ(ungapped, text | ungapped_view);
    EXPECT_RANGE_EQ(gapped, text | gapped_view);
}

TEST_F(modmer_hash_distance_test, ungapped)
{
...
TEST_F(modmer_hash_distance_test, gapped)
{
...
TEST_F(modmer_hash_distance_test, combinability)
{
    ...
    EXPECT_RANGE_EQ(result3_start, text3 | start_at_a | ungapped_view);
    EXPECT_RANGE_EQ(result3_start, text3 | start_at_a | gapped_view);
}
