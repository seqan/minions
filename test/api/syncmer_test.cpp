...
std::vector<int> positions_open{0};
std::vector<int> positions_closed{0, 3};
...
struct iterator_fixture<iterator_type> : public ::testing::Test
{
    ...
    decltype(seqan3::views::syncmer(seqan3::views::kmer_hash(text, seqan3::ungapped{2}), text | kmer_view, 4, {0,3})) test_range =
    seqan3::views::syncmer(vec, text | kmer_view, 4, positions_closed);
};
...
TYPED_TEST(syncmer_view_properties_test, concepts)
{
    ...
    auto v = seqan3::detail::syncmer_view(text | smer_view, text | kmer_view, 4, positions_open);
    compare_types(v);
}
...
TYPED_TEST(syncmer_view_properties_test, different_inputs_kmer_hash)
{
    ...
    EXPECT_RANGE_EQ(closed, seqan3::detail::syncmer_view(text | smer_view, text | kmer_view, 4, positions_closed));
}

TEST_F(syncmer_test, closed_kmer_hash)
{
    EXPECT_RANGE_EQ(result1, seqan3::detail::syncmer_view(text1 | smer_view, text1 | kmer_view, 4, positions_closed));
    EXPECT_RANGE_EQ(result3_closed, seqan3::detail::syncmer_view(text3 | smer_view, text3 | kmer_view, 4, positions_closed));

    auto v1 = text1 | smer_view;
    auto v1_2 = text1 | kmer_view;
    EXPECT_RANGE_EQ(result1, (seqan3::detail::syncmer_view<decltype(v1), decltype(v1_2)>(v1, v1_2, 4, positions_closed)));
    EXPECT_RANGE_EQ(result1_open_t1, (seqan3::detail::syncmer_view<decltype(v1), decltype(v1_2)>(v1, v1_2, 4, {1})));
    auto v2 = text3 | smer_view;
    auto v2_2 = text3 | kmer_view;
    EXPECT_RANGE_EQ(result3_open, (seqan3::detail::syncmer_view<decltype(v2), decltype(v2_2)>(v2, v2_2, 4, positions_open)));
    EXPECT_RANGE_EQ(result3_open_t1, (seqan3::detail::syncmer_view<decltype(v2), decltype(v2_2)>(v2, v2_2, 4, {1})));
    EXPECT_RANGE_EQ(result3_open_t2, (seqan3::detail::syncmer_view<decltype(v2), decltype(v2_2)>(v2, v2_2, 4, {2})));
}

TEST_F(syncmer_test, combinability)
{
    auto stop_at_t = std::views::take_while([] (seqan3::dna4 const x) { return x != 'T'_dna4; });
    EXPECT_RANGE_EQ(result3_closed_stop, seqan3::detail::syncmer_view(text3 | stop_at_t | smer_view, text3 | stop_at_t | kmer_view, 4, positions_closed));

    auto v1 = text3 | stop_at_t | smer_view;
    auto v2 = text3 | stop_at_t | kmer_view;

    EXPECT_RANGE_EQ(result3_open_stop, (seqan3::detail::syncmer_view<decltype(v1), decltype(v2)>(v1, v2, 4, positions_open)));

    auto start_at_a = std::views::drop(6);
    EXPECT_RANGE_EQ(result3_closed_start, (seqan3::detail::syncmer_view{text3 | start_at_a | smer_view, text3 | start_at_a | kmer_view, 4, positions_closed}));
