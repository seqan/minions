...
using iterator_type = std::ranges::iterator_t<decltype(std::declval<seqan3::dna4_vector&>()
                                                       | syncmer_hash<true>(2, 5, seqan3::seed{0}))>;

using closed_iterator_type = std::ranges::iterator_t<decltype(std::declval<seqan3::dna4_vector&>()
                                                      | syncmer_hash<false>(2, 5, seqan3::seed{0}))>;

static constexpr auto open_view = syncmer_hash<true>(2, 5, seqan3::seed{0});
static constexpr auto closed_view = syncmer_hash<false>(2, 5, seqan3::seed{0});
static constexpr auto open_view_14 = syncmer_hash<true>(1, 4, seqan3::seed{0});
static constexpr auto closed_view_14 = syncmer_hash<false>(1, 4, seqan3::seed{0});
...
TEST_F(syncmer_hash_test, open)
{
    ...
    EXPECT_THROW((text3 | syncmer_hash<true>(6, 5, seqan3::seed{0})), std::invalid_argument);
}
...
TEST_F(syncmer_hash_test, closed)
{
    ...
    EXPECT_THROW((text3 | syncmer_hash<false>(6, 5, seqan3::seed{0})), std::invalid_argument);
}
